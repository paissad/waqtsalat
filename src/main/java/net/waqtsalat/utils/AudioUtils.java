...
/**
 * Contains some utilities related to audio files such as playing an audio file for example.
...
public class AudioUtils implements PlayerUtils {
	 ...
	 * This works by examining all available file types.<br /> 
	 * For each type, if the extension this type promisses to handle matches
		...
		new Thread("audioPlayer") {
			public void run() {	
				...
				try {
					...
					line.start();           // Start the line (send the stream to the sound card).

					// Now, we have to write to the line (like any other kind of InputStream)
					int BUFFER  = 4096;
					byte[] data = new byte[BUFFER];
					int bytesRead = 0;
					logger.debug("Playing audio file '{}'", audioFile.getAbsolutePath());
					while((bytesRead = audioInputStream.read(data, 0, data.length)) != -1 && !stop) {
						line.write(data, 0, bytesRead);
					}

					line.close();
					stop = true;
					logger.debug("Finished playing audio file '{}'", audioFile.getAbsolutePath());

				}catch(Exception e) {
					logger.error("Problem while playing audio file" +  audioFile.getAbsolutePath());
					e.printStackTrace();
					try {
