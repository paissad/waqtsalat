...
public class WsParseCommandLine {
	...
	private CmdLineParser.Option automaticOpt; // If we want to compute automatically the prayers time without specifying anything.
	...
	public WsParseCommandLine(String[] args) {
		...
		// Let's do a 1st parse for the only purpose to get the configuration filename.
		try {
		...
		_confFileName  = (String)   parser.getOptionValue(confFileNameOpt);  // _confFileName = null; if -c or --conf are not set !
		try {
			conf = new WsConfiguration(_confFileName);
			_confFileName = conf.getConfFileName();  // _confFileName will get default value "waqtsalat.conf" if _confFileName has a previous null value.
		...
		// Use the option from command line (if specified), otherwise get the value from configuration file, else use default value.
	...
	public void printUsage() {
		System.err.println(""
				...
				+ "                            The default is " + new IpAddress().getDefaultMethodName() + ".\n"
	...
	private void printOptions() {
		System.out.println(""
				+ "\nHelp              : " + _help
				+ "\nDebug             : " + _debug
				+ "\nConfiguration file: " + _confFileName
