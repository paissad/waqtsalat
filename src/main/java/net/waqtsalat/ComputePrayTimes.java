...
public class ComputePrayTimes extends Observable implements Runnable {
	...
	private static int[] _offsets = { 0, 0, 0, 0, 0, 0, 0 }; // { Fajr, Sunrise, Dhuhr, Asr, Sunset, Maghrib, Isha }
	private	Calendar _cal;
	private static double _timezone  = TimeZone.getDefault().getRawOffset() / (1000L * 3600);
	...
	/**
	 * Whether or not we check for a new day ... and then re-compute the
	 * prayer times if we are in a new day (after midnight !).
	 */
		...
		while(true && !_stop) {
			if(_state.equals(State.CHANGED)) {
				...
				try {
					...
					Thread.sleep(10L); // Must be lower than the sleep in the method updateState()
	...
	public void updateState() {
		...
		try {
			Thread.sleep(20L); // Must be higher than the sleep in the run() method !!!
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
	}
	...
	private boolean isNewDay() {
		...
		return (sysDateDAY != _cal.get(Calendar.DAY_OF_YEAR)) ? true : false;
	}
